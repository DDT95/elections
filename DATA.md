...
### Scénarios de vote utile (gauche et centre)

Deux des six scénarios de participation redistribuent des voix entre familles politiques (pas seulement la participation, contrairement aux quatre autres) : « Vote utile à gauche (consolidation autour de LFI) » et « Vote utile au centre (barrage, report des voix LR) », en remplacement des scénarios « Seniors davantage mobilisés » et « Rattrapage de l'abstention » (jugés peu informatifs). Point de départ commun : les scores réels des européennes 2024 (pas de report de participation), avant redistribution.

- **Vote utile à gauche** : hypothèse, **pas une mesure** — la moitié des voix PS/Verts/PCF (familles `social_left`+`ecologist`+`pcf`) se reporte sur LFI (`applyGaucheUtile()` dans `buildDepartmentScenarios()`). S'appuie sur un constat réel mais qualitatif, pas un taux mesuré : en 2022, le vote utile avait fait chuter les scores PS (Hidalgo, 1,75 %) et Verts (Jadot, 4,63 %) au profit de Mélenchon (22,0 %) ; les sondages actuels montrent un fort désir d'union à gauche (8 sympathisants sur 10 selon [Cluster17](https://cluster17.com/2027-la-gauche-divisee-sur-son-candidat-resignee-sur-ses-chances/), 73 % des sympathisants NFP favorables à une candidature unique selon [Harris Interactive/Regards](https://regards.fr/sondage-regards-harris-la-gauche-peut-gagner-en-2027/)). Le taux de 50 % est une hypothèse illustrative, choisie faute de taux mesuré publié — à ajuster si une donnée plus précise apparaît.
- **Vote utile au centre (barrage, report LR)** : les voix Droite (familles `lr`+`other_right`) se reportent 53 % vers le Centre (famille `presidential`), 18 % vers le RN, le reste (29 %) est retiré (abstention) — `applyCentreUtile()`. Taux **mesurés**, pas une hypothèse : [Ipsos-Sopra Steria pour France Télévisions/Radio France/Public Sénat](https://www.ipsos.com/fr-fr/presidentielle-2022/second-tour-profil-des-abstentionnistes-et-sociologie-des-electorats), sociologie des électorats du second tour de la présidentielle 2022 — électeurs de Valérie Pécresse (LR) au 1er tour : 53 % pour Macron, 18 % pour Le Pen au 2nd tour. Donnée **nationale**, pas un report mesuré dans le Val-d'Oise (aucun duel local équivalent n'existe pour le mesurer directement, contrairement au report RN/union de la gauche ci-dessus).
- **Vote des jeunes** (scénario « Territoires jeunes davantage mobilisés », inchangé dans son mécanisme mais son texte cite maintenant des sources) : [Elabe/La Tribune Dimanche, juin 2025](https://www.jean-jaures.org/publication/les-jeunes-emmerdent-ils-le-rassemblement-national/) — chez les 18-24 ans, LFI en tête (29 %) devant le RN (27 %) ; le RN y progresse nettement depuis 2019 (15 % puis 25 % aux européennes 2024).
- **Limites** : ces deux scénarios opèrent sur les scores agrégés du département (pas de simulation commune par commune comme les scénarios de participation), et le « Vote utile à gauche » repose sur un taux choisi, pas mesuré — le texte de chaque scénario le dit explicitement dans l'interface.
