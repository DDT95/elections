...
**Comment vérifier qu'on ne l'a pas réintroduit** : `npm run dev`, ouvrir la page, cliquer sur plusieurs communes différentes, changer d'échelle (commune → bureau de vote → canton → circonscription) en cliquant après chaque changement, changer de scrutin. La carte doit rester peuplée à chaque étape et le tiroir doit s'ouvrir avec les vraies données de l'unité cliquée.
...
- [ ] Cliquer sur une commune, un bureau de vote, un canton ou une circonscription ouvre toujours la fiche avec les bonnes données — jamais un écran vide (§4).
- [ ] Changer d'échelle ou de scrutin ne casse jamais le rendu de la carte.
- [ ] Le survol met en surbrillance sans ouvrir la fiche ; la sélection active reste visuellement distincte du survol.
- [ ] Aucune valeur de vote n'est jamais affichée sans être réelle — un scrutin/échelle incomplet affiche un badge « à compléter », jamais un chiffre inventé.
- [ ] Aucun logo ou emblème de parti politique n'est utilisé — uniquement des couleurs et badges textuels de code de nuance.
- [ ] L'export GeoJSON et l'impression PDF fonctionnent depuis le tiroir de fiche.
