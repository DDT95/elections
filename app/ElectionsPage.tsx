...
function DepartmentAnalysis({ snapshots, socio, communeData, datasets, socioByCommune }: { snapshots: ElectionSnapshot[]; socio: SocioProfile | null; communeData: Record<string, UnitResult>; datasets: Record<string,ElectionCommuneFile>; socioByCommune: Record<string, SocioProfile> }) {
  ...
  return <>
    ...
    <Section title="Impact des scénarios" state="Écart au vote observé"><p className="elec-synthesis-intro">Référence : Européennes 2024, participation 48,3 %. Les valeurs indiquent l’écart produit par le changement de participation.</p><div className="scenario-impact"><header><strong>Hypothèse</strong><strong>Hausse principale</strong><strong>Baisse principale</strong><strong>Effet maximal</strong></header>{scenarios.slice(1).map(sc=>{const changes=sensitivities.map(s=>({...s,delta:(sc.data[s.id]??0)-(baseline[s.id]??0)}));const up=changes.slice().sort((a,b)=>b.delta-a.delta)[0],down=changes.slice().sort((a,b)=>a.delta-b.delta)[0],effect=Math.max(...changes.map(c=>Math.abs(c.delta)));return <article key={sc.label}><div><strong>{sc.label}</strong><small>{sc.note}</small></div><span style={{color:up.color}}>{up.delta>0.05?`${up.label} +${up.delta.toFixed(1)} pt`:"Aucune hausse nette"}</span><span style={{color:down.color}}>{down.delta<-0.05?`${down.label} ${down.delta.toFixed(1)} pt`:"Aucune baisse nette"}</span><b className={effect<.2?"neutral":"marked"}>{effect<.2?"Négligeable":`${effect.toFixed(1)} pt`}</b></article>})}</div></Section>
