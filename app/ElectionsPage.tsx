...
function DepartmentAnalysis({ snapshots, currentKey, socio, communeData, datasets, socioByCommune, deputies, councillors }: { snapshots: ElectionSnapshot[]; currentKey:string; socio: SocioProfile | null; communeData: Record<string, UnitResult>; datasets: Record<string,ElectionCommuneFile>; socioByCommune: Record<string, SocioProfile>; deputies: Record<string, UnitResult> | null; councillors: Record<string, UnitResult> | null }) {
  ...
  return <>
    ...
    <Section title="Scénarios de participation" state="Européennes 2024"><p className="elec-synthesis-intro">Chaque scénario modifie la mobilisation territoriale puis mesure l’effet sur les grandes sensibilités.</p><div className="scenario-cards">{scenarios.map(sc=><article key={sc.label}><header><strong>{sc.label}</strong><p>{sc.explanation}</p></header><div>{sc.effects.map(effect=><span key={effect.id}><b>{effect.label}</b><i><em style={{width:`${Math.min(100,Math.abs(effect.delta)*100)}%`,background:effect.color}}/></i><strong className={effect.delta>=0?"up":"down"}>{effect.delta>=0?"+":""}{effect.delta.toFixed(1)} pt</strong></span>)}</div><footer>{sc.conclusion}</footer></article>)}</div></Section>
  </>;
}
